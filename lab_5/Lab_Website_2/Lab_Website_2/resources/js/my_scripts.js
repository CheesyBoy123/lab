...
function loadStatsPage() {
	...
	//Start at the second row and we only look at cels 2 and 3
	var wins = 0;
	var lose = 0;
	for(i = 2; i < table.rows.length; i++) {
			var buffScore = parseInt(table.rows[i].cells[2].innerHTML);
			var oppScore = parseInt(table.rows[i].cells[3].innerHTML);
...
function loadPlayersPage() {
	var player_sel = document.getElementById('player_selector');
	for(i = 0; i < players.length; i++) {
		var a = document.createElement('a');
		a.innerHTML = players[i].name;
		a.setAttribute('href', '#');
		var s = 'switchPlayers(' + i + ")";
		console.log(s);
		a.setAttribute('onclick', s);
		console.log(players[i].name);
		player_sel.appendChild(a);
	}
}
